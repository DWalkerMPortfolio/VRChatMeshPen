...
### Developer Rationale
In VR, the 3D pen color changing uses a palette above the off-hand as that is an intuitive way to change the color of a paintbrush. Additionally, nearly every input on the Quest controllers is already mapped to something in VRChat.
...
To solve this, we switched to a system where each pen has one `MeshRenderer` that all lines it has drawn are baked into. However this required regenerating the mesh every time it changed. Additionally, it looked slightly worse as the mesh would no longer billboard to face the camera like a line renderer. This version is why the system is frequently referred to as a Mesh 3D Pen.
After copying this system out of the main Meetspace repository, I discovered a method of displaying multiple disconnected lines using a single `LineRenderer` by clipping segments using a shader. This is the system currently in use.
#### Scripts
...
- Marking Line: Used to mark a line targeted by the Eraser or Admin Eraser.
#### Erasers
...
1. Add the `Assets/Prefabs/Mesh Pen` prefab to the scene.
2. Add the `Assets/Prefabs/Mesh Pen Palette` prefab to the scene. There should only be one palette in the scene, regardless of how many pens are present.
3. Assign the `Palette` property on the mesh pen's `Mesh 3D Pen` script to the palette that was just added to the scene.
4. Add the `Assets/Prefab/Mesh Pen Line` prefab to the scene.
...
If you have an object pool of pens (ex: for player to spawn from an inventory), you can use the `GenerateLinesForMesh3DPens` script to quickly assign each pen a line. This will delete any existing lines in the line holder.
...
### To debug line bounds
It can sometimes be useful to verify line bounds are being calculated correctly, especially if erasers do not appear to be targeting lines correctly. To visualize this data:
1. Assign a set of default cube `GameObject`s to the `Bounding Box Displays` property of the `Mesh3DPenLine` script of the line you would like to debug. The number of cubes created is the number of bounding boxes that will be debugged. You may wish to assign a transparent material to the cubes so the lines they encompass will still be visible.
2. Run the game and create the lines you would like to visualize.
3. Select "Debug Display Bounding Boxes" from the component context menu.